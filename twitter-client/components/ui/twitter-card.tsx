...
export const TwitterCard: React.FC<CardProps> = ({
    ...
    return (
        <>
            ...
            >
                <Link className='flex items-start rounded-lg w-10 sm:w-auto' href={`/${data.user?.username}`}>
                    <HoverCard>
                ...
                <div className='flex flex-col w-full '>
                    <div className='flex items-center '>
                        <div className='flex w-full'>
                            <div className='flex items-center '>
                                ...
                                <code>-</code>
                                <p className='mx-1 text-gray-700 font-normal text-sm sm:mx-2'>{data?.createdAt}</p>
                            </div>
                        </div>
                    ...
                    <div className='flex flex-col gap-y-1'>
                        <Link href={`/${data.user?.username}/status/${data.id}`} className='flex w-auto'>
                            <h1 className=' text-black font-normal text-xs sm:text-lg'>{data.title}</h1>
                        </Link>
                        {data.image &&
                            <Link href={`/${data.user?.username}/status/${data.id}`} className='rounded-xl border relative aspect-[4/1.5] md:aspect-[2.4/1.2] overflow-hidden'>
                                <Image
                                    src={data.image}
                                    fill
                                    alt='X-logo'
